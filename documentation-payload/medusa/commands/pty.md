...
### Fork mode stability note

`fork` mode calls `os.forkpty()` from within a multi-threaded process (the agent runs a beacon loop, task threads, and queue-based I/O concurrently). Forking a multi-threaded Python process only duplicates the calling thread; any internal locks (e.g. `Queue` mutexes, CPython's import lock or memory-allocator lock) that happen to be held by other threads at the instant of the fork will remain permanently locked in the child. If the forked REPL then attempts to acquire one of those locks — for example by importing a module or writing to stdout — it can deadlock.

In practice the agent uses no long-held application-level locks, so the window for this is narrow and timing-dependent, but it is non-zero. If you experience a hung REPL after entering `fork` mode, `self` mode provides equivalent introspection capabilities without the fork hazard (at the cost of running inside the live agent process rather than an isolated child).
